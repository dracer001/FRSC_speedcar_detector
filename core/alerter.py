...
(25, 465, 587) on free web services. Raw SMTP — which is what smtplib
uses no matter which mailbox or credentials you point it at — simply
cannot leave the box on the free tier. That's why local testing works
fine and the hosted app always times out on the same code. Port 25 stays
blocked even on paid instances (Render runs on EC2); 465/587 open up
once you're on a paid instance type.

To keep this working on the free tier without upgrading, this module
sends over HTTPS instead of raw SMTP, using Brevo's transactional email
REST API (https://api.brevo.com/v3/smtp/email) whenever a BREVO_API_KEY
env var is set. HTTPS traffic on port 443 isn't affected by the SMTP
port block. Brevo's free tier allows 300 emails/day with a single
verified sender email (no full domain/DKIM setup required):
  1. Sign up at https://www.brevo.com (free, no card required).
  2. Add ids@yunivolt.com (or whichever address you want to send from)
     as a sender and click the verification link Brevo emails you.
  3. Dashboard -> SMTP & API -> API Keys -> generate a key.
...
<<<<<<< HEAD
    ...
    def _send_via_api(self, recipient: str, subject: str, text_body: str, html_body: str) -> Tuple[bool, str]:
        payload = {
            "sender":      {"name": "FRSC Speed Vigil", "email": self.api_sender},
            "to":          [{"email": recipient}],
            "subject":     subject,
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                resp = requests.post(BREVO_API_URL, json=payload, headers=headers, timeout=self.timeout)
                if resp.status_code in (200, 201):
                    with self._lock:
                        self.sent_count += 1
                    log.info(f"[EMAIL:api] Sent to {recipient}: {subject}")
                    return True, "sent"
                last_err = f"HTTP {resp.status_code}: {resp.text[:200]}"
                log.warning(f"[EMAIL:api] Attempt {attempt}/{self.max_retries} failed: {last_err}")
        ...
        with self._lock:
            self.failed_count += 1
        log.error(f"[EMAIL:api] Giving up after {self.max_retries} attempts: {last_err}")
