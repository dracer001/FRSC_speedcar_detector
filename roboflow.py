...
Restructured for maintainability:
  ...
  • Switched detection from the classic single-model endpoint to a
    Roboflow Workflow ("toy-car-detection-uqfuq — Base Workflow"), since
    the retrained model (v6, yolo11n) is only deployed as a workflow —
    see the parsing note above the predictions loop in upload_and_infer().
...
CONFIG = {
    ...
    # Roboflow Workflow (not the classic single-model endpoint — this
    # retrained version is only deployed as a workflow, confirmed via
    # the Deploy tab on app.roboflow.com).
    "workspace_name":     "dracers-workspace",
    "workflow_id":        "toy-car-detection-vtoy-car-detection-uqfuq-6-yolo11n-t1-logic",
    "api_key":            "HrN6gq24W5BypZTSwcgC",

    # TODO-VERIFY: confirm this against the actual class list in the
    # Roboflow project. A live test run only returned "hand" and "box"
    # detections (no toy car in that test image), so this class name
...
#    2. Base64-encode + call Roboflow workflow   — ~200-600 ms
...
def upload_and_infer():
    ...
    # ── FAST: Roboflow workflow inference ─────────────────────
    b64_img = base64.b64encode(img_bytes).decode('utf-8')

    rf_url = f"https://serverless.roboflow.com/infer/workflows/{CONFIG['workspace_name']}/{CONFIG['workflow_id']}"
    ...
    TARGET_CLASS     = CONFIG.get("target_class", "toy_car")
    ...
    try:
        ...
        # Workflow response shape (confirmed from a live run against this
        # workflow — NOT the classic {"predictions": [...]} shape used by
        # the old single-model endpoint):
        #   {
        #     "outputs": [
        #       { "predictions": { "image": {...}, "predictions": [ {class, confidence, ...}, ... ] } }
        #     ],
        #     "profiler_trace": [...]
        #   }
        # outputs is one entry per input image — we only ever send one.
        outputs     = rf_data.get("outputs", [])
        predictions = outputs[0].get("predictions", {}).get("predictions", []) if outputs else []
        ...
        for p in predictions:
            label    = p.get('class')
            raw_conf = float(p.get('confidence', 0))  # workflow returns 0-1 here too
            conf     = round(raw_conf * 100, 2)

            print(f"  → {str(label).upper()} @ {conf}%")

            if label == TARGET_CLASS and raw_conf >= MIN_TOY_CAR_CONF:
                car_detected = True
                is_violation = True
